function convert(content) {
	...
	content = content.replace(/^(import\s+[\w]+(\s*,\s*[\w]+)+)/gm, (match) => {
		return match.replace(/\s*,\s*/g, '\nimport ');
	});
	content = content.replace(/_ba\./g, '_babase.');
	...
	content = content.replace(/\bimport _ba\b/g, 'import _babase');
	content = content.replace(/\bimport ba(\b|\.\w+)/g, 'import babase\nimport bauiv1\nimport bascenev1');
	...
	content = content.replace(/babase\.playsound\(([^,\n]+)(,\s*([^,\n]+))?(,\s*position=([^,\n]+))?\)/gm, '$1.play($3$5)');
	...
	content = content.replace(/bs\.Timer\(([^)]*)\bTimeType\.REAL\b([^)]*)\)/gm, 'babase.AppTimer($1$2)');
